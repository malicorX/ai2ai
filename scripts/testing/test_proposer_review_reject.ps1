...
try {
    $p = @{ Uri = "$BackendUrl/world"; Method = "Get"; TimeoutSec = 5 }; if ($script:BeHeaders.Count -gt 0) { $p.Headers = $script:BeHeaders }; $null = Invoke-RestMethod @p
} catch {
...
$p = @{ Uri = "$BackendUrl/jobs/$jobId"; Method = "Get"; TimeoutSec = 5 }; if ($script:BeHeaders.Count -gt 0) { $p.Headers = $script:BeHeaders }; $j = (Invoke-RestMethod @p).job
...
$balBefore = $null
try {
    $p = @{ Uri = "$BackendUrl/economy/balances"; Method = "Get"; TimeoutSec = 5 }; if ($script:BeHeaders.Count -gt 0) { $p.Headers = $script:BeHeaders }; $balResp = Invoke-RestMethod @p
...
if ($balBefore -ne $null) {
    $balAfter = $null
    try {
        $p = @{ Uri = "$BackendUrl/economy/balances"; Method = "Get"; TimeoutSec = 5 }; if ($script:BeHeaders.Count -gt 0) { $p.Headers = $script:BeHeaders }; $balResp2 = Invoke-RestMethod @p
