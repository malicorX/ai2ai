...
if ($TaskType -eq 'json_list' -and (-not $ver -or $ver -ne "balanced_array")) {
    ...
    Write-Status "Deploy and restart: run .\scripts\deployment\deploy.ps1 -CopyOnly, then on each server: bash ~/ai2ai/scripts/deployment/restart_after_deploy.sh" "Yellow"
    exit 1
}
...
$submitAgent = if ($job.claimed_by) { $job.claimed_by } elseif ($manualClaimedBy) { $manualClaimedBy } else { "agent_2" }
...
Write-Status "  .\scripts\testing\test_run_report.ps1 -JobId $jobId -BackendUrl $BackendUrl" "Gray"
