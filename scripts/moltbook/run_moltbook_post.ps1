...
$scripts = @(
    ...
    "moltbook_solve_challenge.py",
    "moltbook_verify_on_sparky.sh"
)
...
ssh $Target "sed -i 's/\r$//' $RemoteScriptsPath/moltbook_post_on_sparky.sh $RemoteScriptsPath/moltbook_queue_on_sparky.sh $RemoteScriptsPath/moltbook_maybe_post_on_sparky.sh $RemoteScriptsPath/moltbook_prepare_from_run_on_sparky.sh $RemoteScriptsPath/moltbook_cron_post_on_sparky.sh $RemoteScriptsPath/moltbook_verify_on_sparky.sh 2>/dev/null; chmod +x $RemoteScriptsPath/moltbook_post_on_sparky.sh $RemoteScriptsPath/moltbook_queue_on_sparky.sh $RemoteScriptsPath/moltbook_maybe_post_on_sparky.sh $RemoteScriptsPath/moltbook_prepare_from_run_on_sparky.sh $RemoteScriptsPath/moltbook_cron_post_on_sparky.sh $RemoteScriptsPath/moltbook_verify_on_sparky.sh"
...
Write-Host "  Maybe post:   ./moltbook_maybe_post_on_sparky.sh   # checks daily cap + queue + min interval (default 2h); posts one from queue" -ForegroundColor Cyan
...
Write-Host "Cron (every 2h: check queue and post one if allowed):" -ForegroundColor Green
Write-Host "  0 */2 * * * $RemoteScriptsPath/moltbook_prepare_from_run_on_sparky.sh; $RemoteScriptsPath/moltbook_maybe_post_on_sparky.sh >> /tmp/moltbook_cron.log 2>&1" -ForegroundColor Cyan
