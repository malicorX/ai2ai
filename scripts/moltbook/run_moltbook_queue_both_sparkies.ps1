# Add the onboarding posts to the Moltbook queue on both sparky1 and sparky2.
# Cron (or maybe_post) will post from the queue later, respecting 30 min rate limit.
# Usage: .\scripts\moltbook\run_moltbook_queue_both_sparkies.ps1
...
# ---- Sparky1: queue via Python script (queue at ~/.config/moltbook/queue.json)
...
Write-Host "Queuing on sparky1 (Sparky1Agent)..." -ForegroundColor Cyan
$out1 = ssh sparky1 "export MOLTBOOK_TITLE_B64=$title1B64; export MOLTBOOK_BODY_B64=$content1B64; python3 /tmp/queue_add_from_env.py"
Write-Host $out1

# ---- Sparky2: queue via same Python script (queue at ~/.config/moltbook/queue.json)
...
Write-Host "Queuing on sparky2 (MalicorSparky2)..." -ForegroundColor Cyan
$out2 = ssh sparky2 "export MOLTBOOK_TITLE_B64=$title2B64; export MOLTBOOK_BODY_B64=$content2B64; python3 /tmp/queue_add_from_env.py"
Write-Host $out2

Write-Host "`nBoth posts are in the queue. Cron or maybe_post will publish them later (30 min between posts per agent)." -ForegroundColor Green
Write-Host "To list queue: .\scripts\moltbook\run_moltbook_queue_list.ps1 -Target sparky2" -ForegroundColor Cyan
Write-Host "On sparky1, list queue: ssh sparky1 'cat ~/.config/moltbook/queue.json | python3 -m json.tool'" -ForegroundColor Cyan
