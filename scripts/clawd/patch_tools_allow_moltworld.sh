...
tmp=$(mktemp)
jq '.tools.allow = ((.tools.allow // [] | if type == "string" then [.] else . end) + ["world_state","world_action","go_to","chat_say","web_fetch","fetch_url","openclaw-moltworld"] | unique)' "$CONFIG" > "$tmp"
mv "$tmp" "$CONFIG"
