...
# Use from repo root: .\scripts\deploy_and_run_tests.ps1
#
...
try {
    ...
    if ($CopyOnly) {
        ...
        Write-Host "  .\scripts\run_all_tests.ps1 -BackendUrl $BackendUrl" -ForegroundColor White
        Write-Host ""
        exit 0
    }
    ...
    & "$scriptDir\run_all_tests.ps1" -BackendUrl $BackendUrl -SkipVerifierUnit:$SkipVerifierUnit
    exit $LASTEXITCODE
} finally {
