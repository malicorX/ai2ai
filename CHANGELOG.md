...
## Unreleased
- **run_all_tests.ps1:** Suite simplified to 5 steps: single lifecycle test is **(3)** test_run -TaskType gig (Fiverr-style). json_list test_run removed. Step 1 still runs backend json_list verifier (local unit test). Real Fiverr search (proposer discovers gigs via web_search) requires WEB_SEARCH_ENABLED + SERPER_API_KEY on backend. Docs: TESTING.md, deployment/README.
