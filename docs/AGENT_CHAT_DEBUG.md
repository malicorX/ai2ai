...
## 0.3 Control UI Chat: "Lacking necessary details" / web_fetch doesn't run

If you ask in the **Control UI Chat** (e.g. "summarize www.spiegel.de") and the assistant replies **"Your input is lacking necessary details"** or **"exceeds the limitations of the functions I have been given"**, the model is **not** being given any tools in that request.

**Cause:** The **main Chat path** (Control UI → gateway → Ollama) in the current OpenClaw/Clawdbot build **does not send tool definitions** (`tools` / `tool_choice`) to Ollama. So the model never receives `web_fetch` (or any tool) and can't call it. Config (e.g. `tools.allow`, `tools.profile`, SOUL) only affects *which* tools are sent **when** the gateway does send tools; on the Chat UI path it doesn't send them at all. The **jokelord patch** only fixes the **embedded/cron** path (`attempt.ts`), not the Chat session path.

**So:** In the Control UI Chat, **tool calling (including web_fetch) does not work** until the gateway is built/patched so the **main Chat** request builder includes tools. Removing `tools.allow` or updating the SOUL does not fix this.

**Options:**

1. **Use the jokelord-patched gateway** — Apply the [jokelord patch](external-tools/clawd/CLAWD_JOKELORD_STEPS.md) so the **embedded** path sends tools. On many setups, the Control UI Chat and POST /v1/responses **use the same embedded path**; after patching, add `compat.supportedParameters: ["tools", "tool_choice"]` to Ollama models and **install the patched build under Node 22** (OpenClaw 2026.2.x requires Node >=22.12.0 at runtime). Example: `nvm use 22; cd ~/clawdbot-jokelord-build/clawdbot && npm install -g .` then restart the gateway. Then start a **new** chat and try e.g. "summarize www.spiegel.de".
2. **Use a flow that goes through the embedded path** (e.g. a wake/cron that runs the agent via the patched path) — then the model gets tools and can run web_fetch.
3. **Wait for or use an OpenClaw build that sends tools on the Chat path** — e.g. when a release accepts `compat.openaiCompletionsTools` and the main session code uses it. See [OPENCLAW_OLLAMA_TOOL_CALLING_PLAN.md](external-tools/clawd/OPENCLAW_OLLAMA_TOOL_CALLING_PLAN.md).
4. **Patch the gateway source** — find where the main Chat/session builds the Ollama request and add the same tool-routing logic as in the embedded path; then build and install.

Until one of these is in place, expect the Chat UI to respond without running tools (and often with a generic "lacking details" style reply when you ask for a URL).
...
**Assistant shows JSON like `{"name": "exec", "parameters": {"command": "curl ..."}}` but no command runs (no webpage fetched):** The model put the tool call in the **message content** as text. The gateway only **executes** tools when the model returns a **structured `tool_calls`** array in the API response; it does not parse JSON from the message body. So nothing runs. **Config fix:** Ensure the gateway sends the full tool set (including **web_fetch**) so the model can use the proper fetch tool: remove **tools.allow** on the host (run `.\scripts\clawd\run_moltworld_tools_same_as_chat.ps1 -TargetHost sparky2` and sparky1, then restart gateways). Prefer **web_fetch** over exec+curl for URLs. If the model still outputs JSON as text, the **Chat path** may not be sending tool definitions to Ollama (see next paragraph).
...
**Why "it's still not working" — two separate problems**

1. **Hook MoltWorld still in the system message**  
   The gateway only shows that when it **executes** a tool from the MoltWorld plugin. So the plugin is still being loaded. **Cause:** The gateway loads plugins from **`plugins.installs`** in the config (not only `plugins.entries`). That block lists spec, installPath, etc.; as long as it’s there, the gateway will try to load the plugin. **Fix:** Remove the plugin from **both** `plugins.entries` and **`plugins.installs`** in every config file: `~/.openclaw/openclaw.json`, `~/.clawdbot/clawdbot.json`, `~/.clawdbot/openclaw.json`, `~/.openclaw/clawdbot.json`. Use `scripts/clawd/strip_moltworld_config.py` (strips both sections) or the updated `remove_moltworld_plugin_on_sparky.sh`. Also delete the extension dir so the gateway can’t load it; then restart the gateway. Use a **new** chat session.

2. **fetch_url / web_fetch shown as text, no webpage fetched**  
   The model is outputting `fetch_url("...")` or `{"name": "exec", ...}` **in the message content**. The gateway does **not** parse that; it only runs tools when the model returns a **structured `tool_calls`** array in the API response. So nothing runs. **Root cause:** The **main Chat path** (Control UI → gateway → Ollama) in the current build does **not** send tool definitions to Ollama, so the model never gets a real tool-calling request and "plans" in text instead. The jokelord patch fixes the **embedded/cron** path, not the Chat UI path. **Fix:** Use an OpenClaw build that sends tools on the Chat path (e.g. upstream with `compat.openaiCompletionsTools`) or patch the main session request builder in the gateway source. Until then, tool calls from the Control UI chat will not execute.
