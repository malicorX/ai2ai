"""
AI Village Agent — main loop, conversation, planning, events.

Refactored: 2026-02-15
API wrappers and navigation live in agent_tools.py.
Job execution lives in do_job.py.
"""
import os
import random
import time
...
from agent_tools import (
    AGENT_ID, DISPLAY_NAME, WORKSPACE_DIR, ROLE, MAX_CHAT_TO_SCAN,
    COMPUTER_LANDMARK_ID, COMPUTER_ACCESS_RADIUS, HOME_LANDMARK_ID,
    _world_session,
    trace_event, upsert, get_world, get_run_id, world_move,
    board_post, chat_recent, chat_send, chat_topic_get, chat_topic_set,
    economy_balance, economy_balance_of, economy_transfer,
    economy_recent_earnings, economy_record_action,
    memory_append, memory_recent, memory_retrieve,
    rate_importance, memory_append_scored,
    jobs_list, jobs_get, jobs_claim, jobs_submit, jobs_review, jobs_create,
    events_list, event_create, event_invite, event_rsvp,
    web_fetch, web_search, artifact_put,
    opportunities_list, opportunities_update,
    email_template_generate, client_response_simulate,
    _sign, _step_towards, _chebyshev, _get_landmark, _at_landmark, _move_towards,
    _style, load_persona, _read_file, _append_file,
)
from do_job import do_job

USE_LANGGRAPH = agent_tools.USE_LANGGRAPH
if USE_LANGGRAPH:
    from agent_template.langgraph_runtime import llm_chat
    from agent_template.langgraph_agent import run_graph_step

WORLD_API = agent_tools.WORLD_API
PERSONA_FILE = agent_tools.PERSONA_FILE

# PERSONALITY is mutable (set by load_persona). Always read via this module-level
# reference which we refresh after load_persona().
PERSONALITY = agent_tools.PERSONALITY
...
# upsert, get_world, get_run_id → agent_tools
...
# _sign, _step_towards, _chebyshev → agent_tools
...
# _get_landmark .. trace_event → agent_tools
...
# chat_recent .. memory_append_scored → agent_tools
...
# jobs_list .. event_rsvp → agent_tools
...
def maybe_work_jobs() -> None:
    ...
    try:
        submission = do_job(job, cached_balance=_cached_balance)
...
# jobs_create .. _extract_json_array -> agent_tools / do_job.py
...
# chat_topic_get, chat_topic_set -> agent_tools
